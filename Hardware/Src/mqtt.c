...
/*****************对结构体变量进行赋值*****************/
mqtt_struct MY_MQTT = {wifi.ReceiveBuff,0,wifi.SendBuff,0,Init,Connect,SubscribeTopic,PublishData,SentHeart,Disconnect,};
...
char Reception_Comparison(const u8 * Comparison_Buff,u8 Comparison_Len)
{
		...
		if(wifi.FinishFlag)
		{
				...
				printf("%d对比数据:%02x %02x %02x %02x %02x\r\n",Comparison_Len,Comparison_Buff[0],Comparison_Buff[1],Comparison_Buff[2],Comparison_Buff[3],Comparison_Buff[4]);
				printf("%d接收数据:%02x %02x %02x %02x %02x\r\n",Comparison_Len,MY_MQTT.rxbuf[0],MY_MQTT.rxbuf[1],MY_MQTT.rxbuf[2],MY_MQTT.rxbuf[3],MY_MQTT.rxbuf[4]);
				Processing_before_sending();
		}
		return return_value;
}

...
{
	LCD_Dis_String(0,0,"小明物联网1-2 20210528",0x0000,0xffff,2,1);
		for(int i = 0;i < 5; i++)
		{
...
void TouchScreenControl(DevInfo *devinfo)
{
		...
		if(touchnum.x != 65535)
		{
				...
				//12  263   85 314
				if((devinfo->Page == 1)&&(touchnum.x < 85)&&(touchnum.x > 12)&&(touchnum.y <314)&&(touchnum.y >263))
				{
					//进入该作用域  2界面
					devinfo->Page = 2;
				...
				//150 266 213 316 
				else if((devinfo->Page == 1)&&(touchnum.x < 213)&&(touchnum.x > 150)&&(touchnum.y <316)&&(touchnum.y >266))
				{
					//进入该作用域  3界面
					devinfo->Page = 3;
				}
				//180 260 
				else if((devinfo->Page == 2)&&(touchnum.x < 239)&&(touchnum.x > 180)&&(touchnum.y <319)&&(touchnum.y >260))
				{
					//进入该作用域  1界面
					devinfo->Page = 1;
				}
				else if((devinfo->Page == 3)&&(touchnum.x < 239)&&(touchnum.x > 180)&&(touchnum.y <319)&&(touchnum.y >260))
				{
					//进入该作用域  1界面
					devinfo->Page = 1;
				}
...
void DisplayInfo(const DevInfo *devinfo)
{
		...
		switch(devinfo->Page)
		{
			case 1:Auto_Show_Picture(0,0,gImage_main);break;
			case 2:Auto_Show_Picture(180,260,gImage_return);
			LCD_Dis_String(0,50,"农业工程学院",0x0008,0xffee,2,1);
						break;
			case 3:Auto_Show_Picture(180,260,gImage_return);
...
void ReportTask(const DevInfo *devinfo)
{
	...
	if(count > 20)
	{
		count= 0 ;
		u8 tmpbuf[320]={0};
		sprintf((char *)tmpbuf,"{\"method\":\"thing.event.property.post\",\"id\":\"292200613\",\"params\":{\"CurrentTemperature\":%f,\"RelativeHumidity\":%f},\"version\":\"1.0.0\"}",
